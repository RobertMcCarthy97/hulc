...
from calvin_agent.evaluation.calvin_vlm_env import CalvinVLMEnv
...
def main():
    ...
    for checkpoint in checkpoints:
        ...
        # wrap env
        vlm_env = CalvinVLMEnv(env, model, visualize=args.debug, use_model_actions=True)
        
        # run env
        done = False
        obs = vlm_env.reset()
        while True:
            obs, reward, done, info = vlm_env.reset()
            if done:
                obs = vlm_env.reset()
